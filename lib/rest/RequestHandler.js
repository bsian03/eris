...
class RequestHandler {
     ...
     * @arg {Boolean} [options.auth=false] Whether to add the Authorization header and token or not
     * @arg {Object} [options.body] Request payload
     ...
     * @arg {Boolean} [options.formdata=false] Whether to attach the body directly to form data instead of "payload_json" when sending files
     * @arg {String} options.method Uppercase HTTP method
     * @arg {String} [options.reason] Reason to show in the audit logs
     * @arg {String} options.url URL of the endpoint
     * @returns {Promise<Object>} Resolves with the returned JSON data
     */
    request(options, url, auth, body, file, _route, short) {
        if(typeof options === "string") { // This used to be "method", backward compatibility.
            options = {
                method: options
            };
        }
        ...
        if(file !== undefined) {
            options.files = Array.isArray(file) ? file : [file];
        }
        if(_route !== undefined) {
        ...
        const route = options.route || this.routefy(options.url, options.method);
        ...
        return new Promise((resolve, reject) => {
            ...
            const actualCall = (cb) => {
                ...
                try {
                    ...
                    if(options.reason) {
                        let unencodedReason = options.reason;
                    ...
                    if(options.files && options.files.length) {
                        ...
                        options.files.forEach(function(f, i) {
                            if(!f.file) {
                                return;
                            }
                        ...
                        if(options.body) {
                            if(options.formdata) {
                                for(const key in options.body) {
                                    data.attach(key, options.body[key]);
                                }
                            } else {
                                data.attach("payload_json", options.body);
                            }
                        }
                        ...
                        if(options.method === "GET" || options.method === "DELETE") {
                            finalURL += "?" + new URLSearchParams(options.body);
                        } else {
                            // Replacer function serializes bigints to strings, the format Discord uses
                            data = JSON.stringify(options.body, (k, v) => typeof v === "bigint" ? v.toString() : v);
                ...
                req.once("abort", () => {
                    cb();
                    reqError = reqError || new Error(`Request aborted by client on ${options.method} ${options.url}`);
                    reqError.req = req;
                    reject(reqError);
                }).once("error", (err) => {
                    reqError = err;
                ...
                req.once("response", (resp) => {
                    ...
                    if(this._client.listeners("rawREST").length) {
                         ...
                         * @prop {Array<Object>} [request.files] An array of file objects sent in the request
                         * @prop {Buffer} request.files[].file A buffer containing file data
                         * @prop {String} request.files[].name The name of the file
                    ...
                    resp.once("aborted", () => {
                        cb();
                        reqError = reqError || new Error(`Request aborted by server on ${options.method} ${options.url}`);
                        reqError.req = req;
                        reject(reqError);
                    });
                    ...
                    _respStream.on("data", (str) => {
                        ...
                        if(options.method !== "GET" && (resp.headers["x-ratelimit-remaining"] == undefined || resp.headers["x-ratelimit-limit"] == undefined) && this.ratelimits[route].limit !== 1) {
                            this._client.emit("debug", `Missing ratelimit headers for SequentialBucket(${this.ratelimits[route].remaining}/${this.ratelimits[route].limit}) with non-default limit\n`
                                + `${resp.statusCode} ${resp.headers["content-type"]}: ${options.method} ${route} | ${resp.headers["cf-ray"]}\n`
                                + "content-type = " +  + "\n"
                        ...
                        if(resp.statusCode !== 429) {
                            const content = typeof options.body === "object" ? `${options.body.content} ` : "";
                ...
                req.setTimeout(this.options.requestTimeout, () => {
                    reqError = new Error(`Request timed out (>${this.options.requestTimeout}ms) on ${options.method} ${options.url}`);
                    req.abort();
                });
            ...
            if(this.globalBlock && options.auth) {
                this.readyQueue.push(() => {
                    ...
                    this.ratelimits[route].queue(actualCall, options.short);
                });
            } else {
                if(!this.ratelimits[route]) {
                    this.ratelimits[route] = new SequentialBucket(1, this.latencyRef);
